...
def get_layout_style(canvas_data):
    """Determines the grid style based on the number of cells."""
    num_cells = len(canvas_data) if canvas_data else 4
    if num_cells == 3:
        return {"gridTemplateColumns": "1fr 1fr", "gridTemplateRows": "1fr 1fr", "gridTemplateAreas": '"a a" "b c"'}
    elif num_cells == 2:
...
def load_saved_state_and_set_style(saved_data):
    """On page load, copy saved state to session state and set the layout style."""
    state = saved_data if saved_data is not None else [None] * 4
    style = get_layout_style(state)
    return state, style
...
@callback(
    ...
    Output('canvas-drop-area', 'style', allow_duplicate=True),
